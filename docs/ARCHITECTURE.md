...
├─ crates/
│  ├─ core/                   # PURE. no tokio, no reqwest, no fs. only domain + ports.
│  │  ├─ src/domain/          # Provider, UsageWindow, Alarm, CalEvent, Money, ...
│  │  ├─ src/ports/           # trait definitions (the only IO contracts)
│  │  ├─ src/logic/           # reset math, threshold eval, reconciler, normalizers
│  │  └─ src/providers/       # Provider trait + capability blocks (see §5)
│  ├─ adapters/
│  │  ├─ http/                # reqwest impl of HttpPort
│  │  ├─ store/               # sqlx impls of UsageRepo/AlarmRepo/SettingsRepo
│  │  ├─ secrets/             # keyring impl of SecretStore
│  │  ├─ calendar-web/        # Google + MS Graph impls of CalendarPort
│  │  ├─ calendar-native/     # macOS EventKit via sidecar (cfg(target_os="macos"))
│  │  ├─ cookies/             # CookieSource via native sidecar + crypto
│  │  ├─ notify/              # Notifier via tauri-plugin-notification
│  │  └─ loginitem/           # per-OS login-item registration
│  └─ app/                    # Tauri binary: wiring, commands, events, scheduler loop
├─ sidecars/
│  ├─ macos-helper/           # Swift package: EventKit + Safari cookies, JSON-RPC stdio
│  └─ windows-helper/         # .NET helper if/when needed
├─ ui/                        # Svelte + Tailwind front-end (pnpm)
│  ├─ src/
│  └─ src/bindings/           # GENERATED by tauri-specta — do not hand-edit
├─ docs/                      # this folder

...
**Capability:** [§4 Connecting accounts](../PRD.md#4-connecting-accounts-credentials--consent) · **Status:** ✅ done · **Depends on:** 002
...
## Acceptance criteria
- [x] I can **enter** an API key for a provider that requires one, and it takes effect
      **without restarting** the app.
- [x] I can **replace** an existing key and **remove** it.
- [x] The key is stored **only in the OS keychain** — **never** shown again in full, never
      written to logs or the local database.
- [x] An invalid/rejected key shows a **clear error** and does not silently appear connected.
...
- [ADR 0016 — API-key connection model](../adr/0016-api-key-connection-model.md)

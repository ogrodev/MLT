...
## Acceptance criteria
- [x] Each connected provider has a **disconnect** action.
- [x] Disconnecting **removes that provider's tile** from the popover and stops its refresh.
- [x] Any secret the app cached for that provider is **removed from the OS keychain**
      (the app never writes back to a vendor's own credential store).
- [x] Disconnect takes effect **without restarting** the app, and the provider can be
